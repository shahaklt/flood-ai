...
def evaluate_model(name: str, model_fn, X: np.ndarray, y: np.ndarray, groups: np.ndarray) -> dict:
...
def main() -> None:
    ...
    print(f"\nLogistic regression: mean ROC-AUC {logreg_eval['meanRocAuc']:.3f} (+/- {logreg_eval['stdRocAuc']:.3f}), mean Brier {logreg_eval['meanBrier']:.3f}")
    print(f"Gradient boosted trees: mean ROC-AUC {gbm_eval['meanRocAuc']:.3f} (+/- {gbm_eval['stdRocAuc']:.3f}), mean Brier {gbm_eval['meanBrier']:.3f}")
