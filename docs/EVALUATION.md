...
Three plausible threshold-selection objectives were tested via this same nested method, not assumed — maximizing balanced accuracy, F1, and Matthews correlation coefficient. Balanced accuracy is both the metric reported throughout this evaluation and the self-consistent winner for itself, so it's kept as the threshold objective; F1/MCC-optimized thresholds trade balanced accuracy for a different precision/recall balance on the minority class, a legitimate but different deployment choice, not a worse result. Full comparison: `data/models/statewide_ensemble_eval.json`'s `thresholdObjectiveComparison`. Interactive version: `/model`.

Full machine-readable output: `data/models/statewide_eval.json` snapshot at this point in the run history (superseded below by the curvature-included re-fetch).

## Curvature added, statewide re-fetched (`statewide-ensemble-v0.1.1`) — the current best, and an honest null result

A second real statewide fetch added the **terrain curvature** feature (discrete Laplacian of elevation — concave/water-collecting vs. convex/water-shedding terrain) that the first pull didn't have, using NHD+OSM combined water and 100 real negatives sampled per tile. Real yield: **25,095 rows before dropna, 21,956 usable after (curvature is 0% missing), 1,056 real positives (4.8%), 24 real events across 87 real tiles.**

| Model | ROC-AUC | Brier | Balanced accuracy (nested, honest) |
|---|---|---|---|
| Logistic regression | 0.721 | 0.195 | 66.1% |
| Gradient-boosted trees (balanced) | 0.805 | 0.131 | 73.0% |
| LightGBM (tuned) | 0.804 | 0.096 | 74.2% |
| Real average of all three | 0.808 | 0.128 | 74.2% |
| + real isotonic calibration + nested threshold | **0.813** | **0.037** | **73.9%** |

**Honest comparison to the pre-curvature result**: 0.813 ROC-AUC / 73.9% balanced accuracy vs. the earlier 0.821 / 73.8% — curvature and the re-fetch are a **statistical wash**, not an improvement (ROC-AUC actually moved slightly the wrong way, balanced accuracy moved slightly the right way, both within noise for a 24-event dataset). This is reported as the current number because it uses the most complete real feature set available (8 real factors including curvature, matching the shipped baseline's factor count), not because it's a proven step up. The honest conclusion: at 24 real events, this pipeline has plateaued around 0.81-0.82 ROC-AUC / ~74% balanced accuracy regardless of this particular feature addition — more real distinct flood events, not more engineered features from the same events, is what would likely move this number next.

Full machine-readable output: `data/models/statewide_ensemble_eval.json` (current), `data/models/statewide_lightgbm_eval.json`.

**If asked "how accurate is the model": this is the answer** — 0.813 ROC-AUC, 73.9% honest (nested) balanced accuracy, from a real ensemble of three models with real calibration and a real, leakage-checked threshold choice — while still being honest that this is an experimental model on 24 real events, not a validated production classifier, and not what the shipped map uses.
...
Note the *subset* numbers (77.2% balanced accuracy) look higher than the full-dataset ensemble's ~74%, but that subset only spans 12 real events vs. 24 — fewer, less diverse events make cross-validation look easier, not a genuinely better model. The full 24-event result above remains the honestly-reported best, because it's evaluated across the most real, diverse event coverage available.
